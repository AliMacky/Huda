...
class MosqueManager {
    ...
    private func saveToCache(times: [MosquePrayerTimes], mosqueId: String) {
        if let encoded = try? JSONEncoder().encode(times) {
            UserDefaults.standard.set(encoded, forKey: "cached_schedule_\(mosqueId)")
        }
    }
    
    func loadFromCache(mosqueId: String) -> Bool {
        guard let data = UserDefaults.standard.data(forKey: "cached_schedule_\(mosqueId)"),
              let decoded = try? JSONDecoder().decode([MosquePrayerTimes].self, from: data) else {
            return false
        }
    ...
    func searchMosquesByLocation(lat: String, lon: String) async {
        ...
        let urlString = "\(Secrets.masjidalApiBaseUrl)/v3/masjids/proximity?distance=50&per-page=5&lat=\(lat)&long=\(lon)"
        ...
        do {
            ...
            let decodedResponse = try JSONDecoder().decode(MosqueSearchResponse.self, from: data)
    ...
    func fetchMosquePrayerTimes(mosque: MosqueData) async {
        ...
        let fromStr = startOfMonth.formatted(.iso8601.year().month().day().dateSeparator(.dash))
        let toStr = endOfMonth.formatted(.iso8601.year().month().day().dateSeparator(.dash))
        
        let urlString = "\(Secrets.masjidalApiBaseUrl)/v3/masjids/proximity?distance=1&expand=times&per-page=1&lat=\(mosque.latitude)&long=\(mosque.longitude)&date_start=\(fromStr)&date_end=\(toStr)"
        ...
        do {
            ...
            let decodedResponse = try JSONDecoder().decode(RawResponse.self, from: data)
            
            let flattenedList: [MosquePrayerTimes] = decodedResponse.items.flatMap { mosqueItem in
    ...
    func getNextIqama(for date: Date = Date()) -> MosqueIqamaState? {
        ...
        func checkPrayers(for lookupDate: Date) -> MosqueIqamaState? {
            ...
            guard let daySchedule = prayerTimes.first(where: { $0.date == lookupString }) else { return nil }
            ...
            let prayers: [(name: String, athan: String, iqama: String)] = [
                ...
                ("Maghrib", daySchedule.athan.maghrib, daySchedule.iqama.maghrib),
                ...
                if let iqamaDate = fullFormatter.date(from: iqamaString),
                   let athanDate = fullFormatter.date(from: athanString) {
                    ...
                    if iqamaDate > now {
                        ...
                        let timeUntilStr = hours > 0 ? "\(hours)h \(minutes)m" : "\(minutes)m"
                        
                        let offset = Int(iqamaDate.timeIntervalSince(athanDate) / 60)
                        
                        return MosqueIqamaState(
                            prayerName: p.name,
                            iqamaTime: p.iqama,
...
struct RawDay: Decodable {
    let date: String
    let salah: DailyPrayers
    let iqamah: DailyPrayers
}
